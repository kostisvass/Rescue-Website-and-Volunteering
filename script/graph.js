...
//filtrarisma twn dedomenwn vasi imerominiwn
function filterData(data) { 
    //lamvanw to start date
    var startDate = moment($('#startDate').val()).format('YYYY-MM-DD');
    //lamvanw to end date
    var endDate = moment($('#endDate').val()).format('YYYY-MM-DD');
    //xwrismos twn dedomenwn apo to ajax aitima se xwristes katigories
    var offArray = data[0];
    var reqArray = data[1];
    ...
    var filteredData = [
        ...
        //filtrarisma kathe array wste na einai edos twn imerominiwn
    ...
    renderChart(filteredData); //apeikonisi graphimatos ksana me filtrarismena dedomena
}
...
function renderChart(data) {
    ...
    var ctx = document.getElementById('myChart').getContext('2d'); //apoktisi context kamva gia graph
